...
package com.nicodelee.one.archframework;
...
import static com.nicodelee.one.util.LogUtils.LOGE;
import static com.nicodelee.one.util.LogUtils.makeLogTag;
import static com.nicodelee.one.util.PreconditionUtils.checkNotNull;
...
public abstract class ModelWithLoaderManager<Q extends QueryEnum, UA extends UserActionEnum>
   ...
   */
  @VisibleForTesting protected HashMap<Q, DataQueryCallback<Q>> mDataQueryCallbacks =
      new HashMap<>();
  /**
   * Map of callbacks, using the id of the user action as key. This is required because some user
   * actions launch a data query and we can't pass on the
   ...
   * on the callback after the data is read from the cursor.
   */
  @VisibleForTesting protected HashMap<Integer, UserActionCallback<UA>> mDataUpdateCallbacks =
      new HashMap<>();
  /**
   ...
   * android.support.v4.app.LoaderManager.LoaderCallbacks#onLoadFinished(Loader, Object)} has fired,
   * the
   ...
   * on the callback after the data is read from the cursor.
   */
  @VisibleForTesting protected HashMap<Integer, UA> mUserActionsLaunchingQueries = new HashMap<>();
   ...
   */
  @Override public void deliverUserAction(@NonNull UA action, @Nullable Bundle args,
  ...
  @Override public void requestData(@NonNull Q query, @NonNull DataQueryCallback<Q> callback) {
    checkNotNull(query);
    checkNotNull(callback);
   ...
   */
  @VisibleForTesting public void onLoadFinished(Q query, Cursor data) {
    boolean success = readDataFromCursor(data, query);
    if (mDataUpdateCallbacks.containsKey(query.getId()) && mUserActionsLaunchingQueries.containsKey(
        query.getId())) {
    ...
    } else if (mDataQueryCallbacks.containsKey(query) && mDataQueryCallbacks.get(query) != null) {
